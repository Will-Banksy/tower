...
type Instruction = Rc<dyn Fn(&TypedTreeNode, &im::OrdMap<String, TypedTreeNode>, &im::OrdMap<String, Type>, &mut Vec<Value>) -> Result<(), RuntimeError>>;
...
pub fn instructions() -> im::OrdMap<String, (Instruction, StackEffect)> {
	im::ordmap! {
		"__println_str".into() => (
			Rc::new(|typed_tree: &TypedTreeNode, fns: &im::OrdMap<String, TypedTreeNode>, types: &im::OrdMap<String, Type>, stack: &mut Vec<Value>| -> Result<(), RuntimeError> {
			...
			StackEffect::new_popped(im::vector![Type::new_strref(0)])
		)
	}
		// "call".into() => (
